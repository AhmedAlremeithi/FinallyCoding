...
from classes import *
from backend import load_data, save_data, generate_booking_id, generate_ticket_id
...
class TicketBookingGUI:
    ...
    def handle_login(self):
        ...
        if user:
            if admin_code.strip() == "ADMINS12" and isinstance(user, Admin):
                ...
                # Upgrade to admin if admin code was valid
    ...
    def ticket_selection_screen(self):
        ...
        self.ticket_options = [SingleRacePass, WeekendPackage, SeasonMembership, GroupTicket]
        self.ticket_vars = {}

        for ticket_class in self.ticket_options:
            ticket_id = generate_ticket_id(self.system)
            temp_ticket = ticket_class(ticket_id)
            var = tk.IntVar()
